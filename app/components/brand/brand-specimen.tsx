...
export function BrandSpecimen({ direction }: { direction: 1 | 2 | 3 | 4 }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8" data-testid={`brand-specimen-${direction}`}>
      {direction === 4 ? <Direction4Specimen /> : null}
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <Fixture title="Navbar mockup"><div data-testid="fixture-navbar" className="flex items-center justify-between rounded-xl border border-[var(--border)] bg-[var(--card)] px-4 py-3"><span data-brand-label className="font-mono text-[11px] uppercase tracking-[0.18em]">Omega Markets</span><div className="flex gap-2"><Chip active>Trade</Chip><Chip>Batches</Chip><Chip>Account</Chip></div><span data-brand-action-zone className="font-mono text-[10px] uppercase tracking-[0.18em]"><span>markets live</span><span className="h-2.5 w-2.5 rounded-full border border-[var(--border)] bg-transparent" /></span></div></Fixture>
        <Fixture title="Order ticket card"><div data-testid="fixture-order-ticket" className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4"><div className="mb-4 flex gap-2"><Chip active>Buy</Chip><Chip>Sell</Chip></div><div className="grid gap-3 md:grid-cols-2"><Input readOnly value="USDC/EURC" aria-label="Pair" /><Input readOnly value="10,000.00 USDC" aria-label="Amount" /><Input readOnly value="0.9213" aria-label="Price" className="font-tabular" /><Input readOnly value="Limit" aria-label="Order type" /></div><div className="mt-4 flex items-center justify-between text-xs text-[var(--muted-foreground)]"><span>Midpoint settlement</span><Button size="sm">{submitLabel}</Button></div></div></Fixture>
      </div>
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle data-brand-label className="font-mono text-[11px] uppercase tracking-[0.18em] text-[var(--muted-foreground)]">{title}</CardTitle>
      </CardHeader>
