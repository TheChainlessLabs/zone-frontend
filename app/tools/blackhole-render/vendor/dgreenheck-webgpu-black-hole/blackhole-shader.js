...
const createAccretionDiskColor = (uniforms) => Fn(([hitR, hitAngle, time, rayDir]) => {
  ...
  const rawRingOpacity = pow(clamp(turbulence, float(0.0), float(1.0)), uniforms.turbulenceSharpness);
  const softRingOpacity = mix(turbulence, rawRingOpacity, float(0.34));
  ringOpacity.assign(clamp(softRingOpacity, float(0.08), float(0.96)));
