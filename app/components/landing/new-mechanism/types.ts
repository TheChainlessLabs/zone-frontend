// Scene types for the three-step mechanism variant. Same shape as the
// landing scene, but the matching and liquidity phases are one step.

export type SceneStep = "intent" | "matching" | "execution";

export interface SceneState {
  /** Active step index 0..2 and id. */
  index: number;
  ...
  pull: number;
  /** Proof capsule emergence, 0..1 (late phase 2 → 3). */
  proof: number;
  /** Proof capsule x position (SVG units). */
  proofX: number;
  /** Radiation-noise scatter, 0..1 (late phase 2). */
  noise: number;
  /** Tempo finalization blocks slide-in, 0..1 (phase 3). */
  tempo: number;
}
